...
const scheduleTasks = (tasks) => {
  ...
  const scheduled_tasks = ordered_tasks.map(task => {
    let is_late = false;
    let end_task = time + Number(task.runtime);
    end_task > Number(task.delivery) ? is_late = true : is_late = false;
    const scheduled_task = {
      ...task,
      start: time,
      end: end_task,
      is_late: is_late,
      delay: end_task - Number(task.delivery) // tempo entrege - tempo da entrega. 
